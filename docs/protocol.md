...
| `CFG_ELE`          | Provision Elmetron tuning: `CMD,CFG_ELE,wt=<mS/cm>,wdd=<down_duty%>,wdu=<up_duty%>,dto=<s>,ato=<s>,hto=<s>,mto=<s>,cw=<s>,ct=<%>,di=<0\|1>,lal=<0\|1>` (water threshold, winch duty for DOWN and UP separately, descent/ascent/homing/measure timeouts, convergence window+tolerance, direction-invert, limit-active-low). Timeouts ceilinged so the descent safety cap can't be disabled. |
